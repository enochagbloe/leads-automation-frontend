...
function ServiceEditor({ open, onOpenChange, onSaved, service, businessId, defaultCurrency }: { open: boolean; onOpenChange: (open: boolean) => void; onSaved: (service: BusinessService) => void; service: BusinessService | null; businessId: string; defaultCurrency: string }) {
  ...
  const submit = form.handleSubmit(async (values) => {
    try {
      ...
      toast.success(saved.readinessStatus === "DRAFT" || saved.readinessStatus === "INCOMPLETE" ? "Service saved. Some details are still missing." : saved.readinessStatus === "READY_FOR_BOOKING" ? "Service saved. This service is ready for booking." : "Service saved. This service is ready for AI replies.");
      onSaved(saved);
      onOpenChange(false);
    } catch (error) {
      ...
      toast.error(error instanceof ApiError && error.code === "FORBIDDEN" ? "You do not have permission to manage services." : "Could not save service", { description: getApiErrorMessage(error) });
    }
  });
...
export function ServicesPricingPage() {
  ...
  const mutationError = (error: unknown) => {
    ...
    toast.error(error instanceof ApiError && error.code === "FORBIDDEN" ? "You do not have permission to manage services." : "Could not update service", { description: getApiErrorMessage(error) });
  };
