...
function PolicyEditor({ open, onOpenChange, policy, businessId, onLimit }: { open: boolean; onOpenChange: (open: boolean) => void; policy: BusinessPolicy | null; businessId: string; onLimit: (error: ApiError) => void }) {
  ...
  const submit = form.handleSubmit(async (values) => {
    try {
      ...
      toast.success(policy ? "Policy updated" : "Policy added", { description: values.visibility === "CUSTOMER_FACING" && values.isActive ? "BizReply can now use this policy in customer-facing answers." : "The policy has been saved." });
      onOpenChange(false);
    } catch (error) {
      applyApiFieldErrors(error, form.setError);
      if (error instanceof ApiError && error.code === "POLICY_LIMIT_REACHED") onLimit(error);
      toast.error(error instanceof ApiError && error.code === "FORBIDDEN" ? "You do not have permission to manage policies." : "Could not save policy", { description: getApiErrorMessage(error) });
    }
  });
...
export function PoliciesPage() {
  ...
  const mutationError = (error: unknown) => {
    ...
    toast.error(error instanceof ApiError && error.code === "FORBIDDEN" ? "You do not have permission to manage policies." : "Could not update policy", { description: getApiErrorMessage(error) });
  };
