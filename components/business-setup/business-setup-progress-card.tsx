...
export function BusinessSetupProgressCard({ status, onRemindLater }: { status: BusinessSetupStatus; onRemindLater?: () => void }) {
  const next = resolveSetupRoute(status.nextRecommendedStep?.route);
  const title = status.readinessStatus === "NOT_STARTED" ? "Start your business setup" : status.readinessStatus === "READY_FOR_MANUAL_INBOX" ? "Your inbox is ready" : "Complete your business setup";
  const description = status.readinessStatus === "READY_FOR_MANUAL_INBOX"
