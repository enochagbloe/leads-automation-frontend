...
export function DashboardBusinessSetup({ businessId, canManage }: { businessId: string; canManage: boolean }) {
  const setup = useBusinessSetupStatus(businessId);
  const [reminderOpen, setReminderOpen] = useState(false);
  const storageKey = `bizreply_setup_reminder_dismissed_at_${businessId}`;

  useEffect(() => {
    if (!canManage || !setup.data || setup.data.isAiReady || setup.data.completionPercentage >= 70) return;
  ...
  }, [canManage, setup.data, storageKey]);
  ...
  return <><BusinessSetupProgressCard status={setup.data} canManage={canManage} onRemindLater={canManage ? remindLater : undefined} />{canManage && <BusinessSetupReminderDialog open={reminderOpen} onOpenChange={(open) => open ? setReminderOpen(true) : remindLater()} status={setup.data} onRemindLater={remindLater} />}</>;
}
